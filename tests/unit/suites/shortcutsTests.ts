...
import { zoomIn, zoomOut, keydown, toggleSearchMode } from '../../../source/js/shortcuts.js'
import {
    assert,
    assertEqual,
    cloneSections,
...
function installBrowserStubs() {
    // Unit tests run under happy-dom (preloaded via bunfig.toml), so a real
    // document/window is always present here.
    ...
    if (!document.querySelector('.main-content')) {
        ...
        ;(document.querySelector('.main-content') as HTMLElement).style.maxWidth = '800px'
    }

    ;(document.body as any).focused = false
    const nativeFocus = document.body.focus ? document.body.focus.bind(document.body) : null
    ;(document.body as any).focus = function focusWithFlag() {
        this.focused = true
    ...
    try {
        ...
        if (globalThis.navigator.clipboard && typeof globalThis.navigator.clipboard.writeText === 'function') {
            ...
            (globalThis.navigator as any).__shortcutsClipboard = clipboard
        }
    }
...
function createFocus(id: string | null = null, type: string | null = null) {
    return {
        Id: { value: id },
        Type: { value: type },
...
function createKeyEvent({ key, ctrlKey = false, altKey = false, shiftKey = false }: any) {
    return {
        key,
        ctrlKey,
        altKey,
...
export async function runShortcutsTests(onProgress: any) {
    if (_cachedResult) {
    ...
    await test('Ctrl+Alt+w toggles wide mode and consumes event', async () => {
        ...
        await handle(event)
        assertEqual((document.querySelector('.main-content') as HTMLElement).style.maxWidth, 'none', 'First toggle should enable wide mode')

        await handle(event)
        assertEqual((document.querySelector('.main-content') as HTMLElement).style.maxWidth, '800px', 'Second toggle should restore default width')
        assert(event.defaultPrevented, 'Handled shortcut should prevent default')
    })
    ...
    await test('Escape clears focused node and focuses body', async () => {
        ...
        assert((document.body as any).focused, 'Escape should move focus to document body')
    })
    ...
    await test('ArrowUp on first child in zoomed view clears focus', async () => {
        ...
        assert((document.body as any).focused, 'ArrowUp on first zoomed child should move focus to body')
    })
    ...
    await test('ArrowDown on last child in zoomed view clears focus', async () => {
        ...
        assert((document.body as any).focused, 'ArrowDown on last zoomed child should move focus to body')
    })
    ...
    await test('Backspace on empty description switches focus type back to text', async () => {
        addChild('root', { id: 'A', text: 'A', description: '' })
    ...
    await test('Enter on focused description does not create a new node', async () => {
        addChild('root', { id: 'A', text: 'A', description: 'desc' })
        const focus = createFocus('A', 'description')
        const handle = keydown(focus)
        const event = createKeyEvent({ key: 'Enter' })
    ...
    await test('Ctrl+c copies focused subtree VMD to clipboard', async () => {
        ...
        const copied = (globalThis.navigator.clipboard && (globalThis.navigator.clipboard as any).text) ||
            ((globalThis.navigator as any).__shortcutsClipboard && (globalThis.navigator as any).__shortcutsClipboard.text) || ''
