import { mergeDocuments } from '../../../source/js/sync.js'
import {
    assert,
    assertEqual,
    cloneSections,
...
export async function streamSyncTests(onProgress: any) {
...
function baseNodes(extra: any[] = []): any[] {
    return [
...
await test("Root node is never deleted even if absent from one side", () => {
    ...
    const remote: any[] = []  // no root in remote
    const { merged } = mergeDocuments(local, remote, SYNCED_AT)
