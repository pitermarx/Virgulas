...
import { groupedTasks, hasOverdueTasks, pendingTaskCount } from '../../../source/js/tasks.js'
import {
    assert,
    assertEqual,
    cloneSections,
...
export async function streamTasksTests(onProgress: any) {
    return streamCompletedSections(cloneSections(sections), onProgress, 10)
}

// ─── helpers ──────────────────────────────────────────────────────────────────

function node(id: string, text: string, overrides: any = {}) {
    return {
        id,
        parentId: 'root',
        text,
