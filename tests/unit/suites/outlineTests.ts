...
function assertChildren(nodeId: string, expectedIds: string[]) {
...
export async function streamOutlineTests(onProgress: any) {
...
function buildTree() {
  ...
  addChild('B', { id: 'D', text: 'D' })
  addChild('B', { id: 'E', text: 'E' })
}
...
await test("peek() returns a plain object, not the node itself", () => {
  ...
  assert((peeked as unknown) !== root, "peek() should return a new object")
...
await test("addChild with explicit id, text, description, open", () => {
  const node = addChild('root', { id: 'X', text: 'hello', description: 'desc', open: false })
...
await test("addChild to non-root parent", () => {
  const parent = addChild('root', { id: 'P' })
  const child = addChild('P', { id: 'CH' })
...
await test("updateNode with only text leaves description unchanged", () => {
  addChild('root', { id: 'A', description: 'keep me' })
...
await test("updateNode with only description leaves text unchanged", () => {
  addChild('root', { id: 'A', text: 'keep me' })
...
await test("root node serialized with null parentId", () => {
  const parsed = JSON.parse(outline.serialize())
  assert(!parsed.nodes.find((n: any) => n.id === 'root').parentId, "Root parentId should be null or undefined in serialized output")
})

await test("serialize round-trip preserves open state", () => {
  addChild('root', { id: 'A', open: false })
  addChild('root', { id: 'B', open: true })
  const parsed = JSON.parse(outline.serialize())
  assertEqual(parsed.nodes.find((n: any) => n.id === 'A').open, false, "Node A open should be false")
  assertEqual(parsed.nodes.find((n: any) => n.id === 'B').open, undefined, "Node B open should be undefined (true is default)")
})
...
async function checkVersion(version: number, fn: () => any) {
  fn()
...
await test("addChild increments version", async () =>
  await checkVersion(1, () => addChild('root')))
...
await test("Nested list", () => {
  addChild('root', { id: 'A', text: 'A' })
  addChild('A', { id: 'B', text: 'B' })
  addChild('A', { id: 'C', text: 'C' })
  const vmd = outline.getVMD()
  assertEqual(vmd, "- A\n  - B\n  - C\n", "Nested VMD")
})

await test("Collapsed node with children uses + bullet", () => {
  addChild('root', { id: 'A', text: 'A', open: false })
  addChild('A', { id: 'B', text: 'B' })
  const vmd = outline.getVMD()
...
await test("Description lines are indented under their node", () => {
  addChild('root', { id: 'A', text: 'A', description: 'line one\nline two' })
  const vmd = outline.getVMD()
...
await test("Basic parsing creates child nodes", () => {
  const parent = addChild('root', { id: 'P', text: 'P' })
  outline.setVMD("- X\n- Y\n", 'P')
...
await test("+ bullet sets open to false", () => {
  const parent = addChild('root', { id: 'P' })
  addChild('P', { id: 'DUMMY' }) // child so + renders
...
await test("- bullet sets open to true", () => {
  const parent = addChild('root', { id: 'P', open: false })
  outline.setVMD("- Open\n", 'P')
...
await test("addChild with duplicate id is rejected and returns undefined", () => {
  addChild('root', { id: 'A' })
  const result = addChild('root', { id: 'A' })
...
await test("deleteNode clears text instead of removing the only child of the zoom root", () => {
  addChild('root', { id: 'A', text: 'keep structure' })
  outline.deleteNode('A')
...
await test("addChild without parentId adds to the current zoom root", () => {
  addChild('root', { id: 'A' })
  outline.zoomIn('A')
  const child = addChild(undefined, { id: 'B' })
...
function buildNavTree() {
  ...
  addChild('root', { id: 'B', text: 'B' })
  addChild('root', { id: 'C', text: 'C' })
  addChild('A', { id: 'D', text: 'D' })
  addChild('A', { id: 'E', text: 'E' })
}
...
await test("Multiple rapid updates are batched into a single version increment", async () => {
  ...
  addChild('root', { text: '1' })
  addChild('root', { text: '2' })
  addChild('root', { text: '3' })
...
await test("New nodes have lastModified = 0", () => {
  const { id } = addChild('root', { text: 'Alpha' })
...
await test("update() sets lastModified when text changes", () => {
  const { id } = addChild('root', { text: 'Original', lastModified: 0 })
...
await test("update() sets lastModified when description changes", () => {
  const { id } = addChild('root', { text: 'Node', lastModified: 0 })
  const node = outline.get(id)!
...
await test("update() does NOT change lastModified when nothing changes", () => {
  const { id } = addChild('root', { text: 'Static', lastModified: 0 })
  const node = outline.get(id)!
...
await test("toggleOpen() sets lastModified", () => {
  const { id } = addChild('root', { text: 'Togglable', lastModified: 0 })
  const node = outline.get(id)!
  assertEqual(node.lastModified, 0, "Should start at 0")
  const before = Date.now()
  node.toggleOpen()
...
await test("removeChild() sets lastModified on parent", () => {
  ...
  const { id: childId } = addChild('root', { text: 'Child' })
...
await test("serialize() includes lastModified when non-zero", () => {
  ...
  const found = obj.nodes.find((n: any) => n.id === 'ts-node')
...
await test("serialize() omits lastModified when 0", () => {
  ...
  addChild('root', { id: 'no-ts', text: 'No TS', lastModified: 0 })
  const raw = outline.serialize()
  const obj = JSON.parse(raw)
  const found = obj.nodes.find((n: any) => n.id === 'no-ts')
...
await test("deserialize() reads lastModified from payload", () => {
  ...
  const rootNode = obj.nodes.find((n: any) => n.id === 'root')
...
await test("checkboxToggleDone toggles false ↔ true and never removes task state", () => {
  addChild('root', { id: 'C', text: 'cb', done: false })
...
await test("checkboxToggleDone on a recurring task advances due date instead of becoming done", () => {
  addChild('root', { id: 'Rec1', text: 'Pay bill due:2026-08-28 rec:1m', done: false })
...
await test("toggleDone (Ctrl+Enter) on a recurring task advances due date instead of becoming done", () => {
  addChild('root', { id: 'Rec2', text: 'Pay bill due:2026-08-28 rec:1m', done: false })
...
await test("recurring task with no due date behaves like a normal task (rec is inert)", () => {
  addChild('root', { id: 'Rec3', text: 'Water plants rec:1w', done: false })
...
await test("unchecking a recurring done task behaves like a normal toggle", () => {
  addChild('root', { id: 'Rec4', text: 'Pay bill due:2026-08-28 rec:1m', done: true })
...
await test("removeTaskMark resets done to null", () => {
  addChild('root', { id: 'R', text: 'rmtask', done: true })
...
await test("getAllTasks includes only nodes with done !== null", () => {
  addChild('root', { id: 'P1', text: 'plain' })
  addChild('root', { id: 'T1', text: 'undone', done: false })
  addChild('root', { id: 'T2', text: 'done', done: true })
...
await test("getVMD encodes unchecked task with [ ]", () => {
  addChild('root', { id: 'V1', text: 'Buy milk', done: false })
...
await test("setRootVMD round-trip preserves done=true", () => {
  addChild('root', { id: 'RT2', text: 'Done thing', done: true })
  const vmd = outline.getVMD('root')
  outline.setRootVMD(vmd)
